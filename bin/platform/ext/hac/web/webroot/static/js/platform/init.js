	...
	$('#dumpConfiguration').click(function(){
		...
		$.ajax({
			...
			success: function (data) {
                ...
                preContainer.text(JSON.stringify(data, undefined, 2));

				$('#updInitConfigContainer').html("");
				$( "#updInitConfigContainer" ).dialog({
					  title: "Configuration: "
					});
				$('#updInitConfigContainer').append(preContainer);
		...
		$.ajax({
			...
			success: function(data) {
				if (data.isInitializing) {
					...
					$('#master').text(data.master ? "true" : "false");
	...
	function renderProjectOptionsUI(projectDataParam, projectDataName) {
		...
		if (!$.isEmptyObject(projectDataParam)) {
			...
			$.each(projectDataParam, function(key, parameter) {
                const parameterId = $("<div/>").text(`${projectDataName}_${parameter.name}`).html();
                const dt = $('<dt/>').text(parameter.label);
                const dd = $('<dd/>');
				...
				if (parameter.values) {
					...
					$.each(parameter.values, function(key, selected) {
                        const k = $("<div/>").text(key).html();
                        const option = $('<option>', {value: k}).text(k);
                        if (selected) {
                    ...
                    const inputDefaultValue = $("<div/>").text(parameter["default"]).html();
