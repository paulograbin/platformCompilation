		...
		function() {
			...
			$.ajax({
				...
				success : function(data) {
					...
					for (const pos in data.scopes) {
                        if (!data.scopes.hasOwnProperty(pos)) {
                            continue;
                        }
						const entry = data.scopes[pos];
						const option = $('<option/>').val(entry.key).text( entry.value);
						$('#scopeJars').append(option.clone());
...
function getResults(requestType) {
	...
	if (requestType == "jars") {
		...
		dataObj.option = $('#duplicatesOnly').is(':checked') ? true : false;
		...
		dataObj.option = $('#allClasses').is(':checked') ? true : false;
	}
