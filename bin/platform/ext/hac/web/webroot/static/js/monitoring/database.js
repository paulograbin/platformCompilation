	...
	$("body").on('click', '.resetButton',function(e) {
		...
		$.ajax({
			...
			success: function(data) {
                $('#ds_' + $("<div/>").text(data.dsId).html()).fadeOut('fast', function (f) {
                    $(this).remove();
                });
		...
		$.ajax({
			...
			success: function(data) {
                ...
                if (data.success) {
                    ...
                    $('#totalQueries').text(data.totalQueries);
                    ...
                    $('#analyzeLog').text(buttonTextAnalyze);
                }
                else{
                    hac.global.error(data.error);
                }
	...
	$.ajax({
		...
		success: function(data) {
			...
			$('#loggingSpinnerWrapper').fadeOut('fast', function(e){
				...
				$('#logFilePath').text(data.logFilePath);
				$('#logFileSize').text(data.logFileSize);
...
function appendDataSourceInfo(datasource)
{
    ...
    const dsId = $("<div/>").text(datasource.dsId).html();
    ...
    const fieldset = $('<fieldset>', {id: `ds_${dsId}`});
...
{
	var dsInUse = (datasource.numInUse === 1) ? " connection" : " connections";
	var dsOpen = (datasource.numPhysicalOpen === 1) ? " connection" : " connections";
	var dsRoOpen = (datasource.numReadOnlyOpen === 1) ? " connection" : " connections";
