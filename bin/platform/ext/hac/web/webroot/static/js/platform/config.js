$(function() {
    ...
    function isNotValidConfigKey(string) {
        var regex = new RegExp("\\s+"); // java property pattern, ex.: 'this.is.my.property.key'
        return regex.test(string);
    }

    function propertyExists(property){
        const propertyId = '#' + property.replaceAll('.','\\.');
        return table.fnGetNodes(propertyId) != null;
    }
    ...
    $("body").on("click", '#addButton', function(e) {
        ...
        {
            hac.global.error("Property already exist. Please edit existing property");
            $(this).removeAttr('disabled');
            return;
        }
        ...
        rowNew(row);
        hac.global.notify("Key '" + configKeyInfo + "' was added, but not saved.");
        $(this).removeAttr('disabled');
    });
