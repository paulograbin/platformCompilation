...
public class ClassificationAttributeTranslatorTest extends HybrisJUnit4TransactionalTest
{
	...
	private ClassificationAttributeTranslator classificationAttributeTranslator;
	...
	private String getCLColumn(final ClassificationAttribute attr, final String delimiter, final String dateformat)
	{
		...
		{
			stringBuilder.append(",")
			             .append(ImpExConstants.Syntax.Modifier.DATEFORMAT)
			             .append("='")
			             .append(dateformat)
			             .append("'");
		}
	...
	{
		final StringBuilder stringBuilder = new StringBuilder();
		stringBuilder.append(ImpExConstants.Syntax.SPECIAL_COLUMN_PREFIX)
		             .append(assignment.getClassificationAttribute().getCode());
		stringBuilder.append("[");
		stringBuilder.append("translator='").append(ClassificationAttributeTranslator.class.getName()).append("'");
		stringBuilder.append(",")
		             .append(ClassificationAttributeTranslator.Modifiers.SYSTEM)
		             .append("='")
		             .append(assignment.getClassificationClass().getSystemVersion().getClassificationSystem().getId())
		             .append("'");
		stringBuilder.append(",").append(ClassificationAttributeTranslator.Modifiers.VERSION).append("='")
	...
	public void testSameAttributeDifferentClasses() throws ImpExException
	{
		...
		final String header = "INSERT_UPDATE Product;" + //
				...
				getCLColumn(
						attr2) + ";" + // let attr2 match dynamically -> should only find ccat12.attr2 since p3 is NOT assigned to ccat2
		...
		assertEquals(Collections.EMPTY_LIST, container.getFeature(ccat12_attr2)
		                                              .getValuesDirect()); // since p3 is NOT assigned to ccat12 first attr2 column should have no effect !
	...
	public void testProductFeatureValueExport() throws ConsistencyCheckException, Exception
	{
		...
		try
		{
			...
			final String expected = "INSERT ProductFeature;qualifier[unique=true];product;classificationAttributeAssignment;language;value[translator=de.hybris.platform.catalog.jalo.classification.impex.ProductFeatureValueTranslator]"
					...
					+ product2.getPK()
					+ ";"
					+ ccat12.getAttributeAssignment(attr4)
					        .getPK() + ";;enum,TestSys,2.0,val1" + CSVConstants.DEFAULT_LINE_SEPARATOR;
	...
	@Test
	public void testLocale() throws ImpExException
	{
		final String lines = "INSERT_UPDATE Product;" + Product.CODE + "[unique=true];"
		                + CategoryConstants.Attributes.Product.SUPERCATEGORIES + "(" + Category.CODE + ");"
		                + ImpExConstants.Syntax.SPECIAL_COLUMN_PREFIX + attr6.getCode() + "[" + "translator='"
		                + ClassificationAttributeTranslator.class.getName() + "',"
		                + ClassificationAttributeTranslator.Modifiers.SYSTEM + "='" + sys.getId() + "',"
		                + ClassificationAttributeTranslator.Modifiers.VERSION + "='" + sysVer.getVersion() + "',"
		                + ImpExConstants.Syntax.Modifier.LANGUAGE + "='de'" + "]\n" + "; p1 ;" + ccat12.getCode()
		                + "; test DE\n";

		final String expr = ImpExConstants.Syntax.SPECIAL_COLUMN_PREFIX + attr6.getCode() + "[" + "translator='"
		                + ClassificationAttributeTranslator.class.getName() + "',"
		                + ClassificationAttributeTranslator.Modifiers.SYSTEM + "='" + sys.getId() + "',"
		                + ClassificationAttributeTranslator.Modifiers.VERSION + "='" + sysVer.getVersion() + "',"
		                + ImpExConstants.Syntax.Modifier.LANGUAGE + "='de'" + "]";

		classificationAttributeTranslator = new ClassificationAttributeTranslator();
		final ImpExImportReader impExReader = createImpExImportReader(lines);
		impExReader.readLine();
		final HeaderDescriptor header = impExReader.getCurrentHeader();
		final SpecialColumnDescriptor columnDescriptor = new SpecialColumnDescriptor(3, header, expr);
		classificationAttributeTranslator.init(columnDescriptor);
		final Locale locale = classificationAttributeTranslator.locale;

		// Verify locale
		assertEquals(german.getIsocode(), locale.getLanguage());
	}
