...
public class NewSyncRestrictionsTest extends HybrisJUnit4Test
{
	...
	public void setUp() throws ConsistencyCheckException
	{
		...
		final UserGroup group = UserManager.getInstance().createUserGroup("syncGroup");
		group.addMember(userSync);
		TypeManager.getInstance().createRestriction("testRestriction", group,
				TypeManager.getInstance().getComposedType(Product.class),
				"{item.code} = 'prod1'");
	}
	...
	public void testNewSyncWithRestrictions() throws Exception
	{
		...
		assertEquals("Wrong CronJob result! Should be set to 'Success'!", testCronJob.getResult(),
				testCronJob.getSuccessResult());
