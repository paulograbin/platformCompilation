...
public class TaskDAOLockingTest extends ServicelayerBaseTest
{
	...
	public void lockShouldUpdateEntitiesWithTransaction()
	{
		...
		lockShouldUpdateEntitiesWithTransactionLogic(taskDAO);
	}

	@Test
	public void lockShouldUpdateEntitiesWithTransactionComplianceLogic()
	{
		final MockTaskDAO taskDAO = spy(new MockComplianceLogicTaskDAO(Registry.getCurrentTenant()));
		lockShouldUpdateEntitiesWithTransactionLogic(taskDAO);
	}

	public void lockShouldUpdateEntitiesWithTransactionLogic(final MockTaskDAO taskDAO)
	{
		taskDAO.lock(1L, 1L);
		...
		verify(taskDAO, times(2)).consumeConditionsAndClearCache(any());
	...
	public static class MockTaskDAO extends TaskDAO
	{
		...
		@Override
		void consumeConditionsAndClearCache(final Long pk)
		{
			markRunningTx(Transaction.current().getObjectID(), Transaction.current().isRunning());
		}
	}

	public static class MockComplianceLogicTaskDAO extends MockTaskDAO
	{
		public MockComplianceLogicTaskDAO(final Tenant t)
		{
			super(t);
		}
		...
		{
			return super.getConditionConsumeQuery();
		}
	}
}
