...
import de.hybris.platform.task.impl.RuntimeConfigHolder.BooleanTaskEngineProperty;
...
public class RuntimeConfigHolderTest
{
	...
	private static final BooleanTaskEngineProperty BACKGROUND_BOOLEAN_PROPERTY = RuntimeConfigHolder.booleanProperty(
			"backgroundBooleanProperty",
			Boolean.FALSE);
	...
	{
		//given
		final BooleanTaskEngineProperty property = RuntimeConfigHolder.booleanProperty("booleanProperty", Boolean.FALSE);
		final RuntimeConfigHolder configHolder = new TestRuntimeConfigHolder(ImmutableMap.of("booleanProperty", "true"));

		//when
		final Boolean value = configHolder.getProperty(property);
	...
	@Test
	public void shouldReturnDefaultBooleanProperty()
	{
		//given
		final BooleanTaskEngineProperty property = RuntimeConfigHolder.booleanProperty("booleanProperty", Boolean.TRUE);
		final RuntimeConfigHolder configHolder = new TestRuntimeConfigHolder(Collections.emptyMap());
	...
	@Test
	public void shouldReturnUpdatedBooleanProperty()
	{
		//given
		final Map<String, String> mockConfig = new HashMap<>();
		final BooleanTaskEngineProperty property = RuntimeConfigHolder.booleanProperty("booleanProperty", Boolean.FALSE);
		final RuntimeConfigHolder configHolder = new TestRuntimeConfigHolder(mockConfig);
		mockConfig.put("booleanProperty", "true");

		//when
		final Boolean firstValue = configHolder.getProperty(property);
		mockConfig.put("booleanProperty", "false");
		final Boolean secondValue = configHolder.getProperty(property);
		mockConfig.put("booleanProperty", "TRUE");
		final Boolean thirdValue = configHolder.getProperty(property);
		mockConfig.put("booleanProperty", "FALSE");
		final Boolean fourthValue = configHolder.getProperty(property);
		mockConfig.put("booleanProperty", "non-parsable value");
		final Boolean nonParsableValue = configHolder.getProperty(property);
		mockConfig.remove("booleanProperty");
		final Boolean noValue = configHolder.getProperty(property);

		//then
		assertThat(firstValue).isEqualTo(Boolean.TRUE);
		assertThat(secondValue).isEqualTo(Boolean.FALSE);
		assertThat(thirdValue).isEqualTo(Boolean.TRUE);
		assertThat(fourthValue).isEqualTo(Boolean.FALSE);
		assertThat(nonParsableValue).isEqualTo(Boolean.FALSE);
		assertThat(noValue).isEqualTo(Boolean.FALSE);
	}
	...
	@Test
	public void shouldEvaluateBooleanProperty() throws Exception
	{
		//given
		final BooleanTaskEngineProperty property = RuntimeConfigHolder.booleanProperty("booleanProperty", Boolean.FALSE);
		final RuntimeConfigHolder configHolder = new TestRuntimeConfigHolder(ImmutableMap.of("booleanProperty", "#cores == 5"),
				5);
		setBackgroundProperties(configHolder);
	...
	private void setBackgroundProperties(final RuntimeConfigHolder configHolder)
	{
		...
		configHolder.getProperty(BACKGROUND_BOOLEAN_PROPERTY);
	...
	private void checkBackgroundProperties(final RuntimeConfigHolder configHolder)
	{
		...
		assertThat(configHolder.getProperty(BACKGROUND_BOOLEAN_PROPERTY)).isEqualTo(Boolean.FALSE);
	}
