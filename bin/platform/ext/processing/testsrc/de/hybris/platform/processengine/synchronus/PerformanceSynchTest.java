...
import de.hybris.platform.processengine.XmlInputFactoryCreator;
...
{
	private static final Logger LOG = LoggerFactory.getLogger(PerformanceSynchTest.class);
	...
	@Test
	public void performanceAsynch() throws InterruptedException
	{
		generateAndRunBPM(false);
	}

	@Test
	public void performanceSynch() throws InterruptedException
	{
		generateAndRunBPM(true);
	}

	private void generateAndRunBPM(final boolean canJoinPrivousNode) throws InterruptedException
	{
		LOG.info("GENERATING TASKS canJoinPreviousNode: " + canJoinPrivousNode);
		...
		context.registerProcesses(uniqueIds, canJoinPrivousNode);
		...
		LOG.info("Min: " + DurationFormatUtils.formatDurationHMS(stats.getMin()));
		LOG.info("Avg: " + DurationFormatUtils.formatDurationHMS(Math.round(stats.getAverage())));
		LOG.info("Max: " + DurationFormatUtils.formatDurationHMS(stats.getMax()));

		LOG.info("TOTAL TIME " + Duration.between(startTime, Instant.now()));
		context.clear();
	}


	private void runBusinessProcess(final BusinessProcessModel process)
	{
	...
	private String extractCodeFromContent(final String content)
	{
		try
		{
			final XMLEventReader reader = XmlInputFactoryCreator.createXMLInputFactory().createXMLEventReader(new StringReader(content));
