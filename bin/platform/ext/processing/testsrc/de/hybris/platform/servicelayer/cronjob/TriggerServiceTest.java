...
public class TriggerServiceTest extends ServicelayerTransactionalBaseTest
{
	 ...
	 *
	 * @param job        the <code>JobModel</code> the <code>TriggerModel</code> should be assigned to
	 * @param second     the second at which the <code>TriggerModel</code> should fire (0-59 or -1 for every second)
	 * @param minute     the minute at which the <code>TriggerModel</code> should fire (0-59 or -1 for every minute)
	 * @param hour       hour at which the <code>TriggerModel</code> should fire (0-23 or -1 for every hour)
	 * @param day        the day of month the <code>TriggerModel</code> should fire (0-31 or -1 for every day)
	 * @param month      the month at which the <code>TriggerModel</code> should fire (0-11 or -1 for every month)
	 * @param year       the year when the <code>TriggerModel</code> should fire (-1 for every year)
	 * @param daysOfWeek a <code>List</code> of
	 * @param relative   true if time values should be considered relative to each other or false if not
	...
	private TriggerModel configureNewTrigger(final JobModel job, final int second, final int minute, final int hour,
	                                         final int day, final int month, final int year, final List daysOfWeek,
	                                         final boolean relative)
	{
		...
		trigger.setActivationTime(triggerService.getNextTime(trigger,
				Calendar.getInstance(i18nService.getCurrentTimeZone(),
						i18nService.getCurrentLocale())).getTime());
		...
		for (int i = 0; i < 5; i++)
		{
			...
			if (LOG.isDebugEnabled())
			{
				LOG.debug("year day/Month h:min " + now.get(Calendar.YEAR) + " " + now.get(Calendar.DAY_OF_MONTH) + "/"
						+ now.get(Calendar.MONTH) + " " + now.get(Calendar.HOUR_OF_DAY) + ":" + now.get(
						Calendar.MINUTE));
			}
	...
	public void shouldActivateTwiceWithoutErrorWithDateRangeSetInPast()
	{
		...
		final TriggerModel trigger = configureNewTrigger(job, -1, -1, -1, -1, -1, -1, null, false);
		trigger.setActive(Boolean.TRUE);
		trigger.setDateRange(new StandardDateRange(Date.from(Instant.now().minus(2, ChronoUnit.DAYS)),
				Date.from(Instant.now().minus(1, ChronoUnit.DAYS))));
		modelService.save(trigger);

		((DefaultTriggerService) triggerService).activateForTest(trigger, System.currentTimeMillis() + 1000000);
		modelService.refresh(trigger);
		final boolean activate = ((DefaultTriggerService) triggerService).activateForTest(trigger,
				System.currentTimeMillis() + 2000000);
		assertThat(activate).isFalse();
	}

	@Test
	public void shouldPopulateNodeGroupFromJobToCronJob()
	{
		// creates a trigger who triggers every 10 seconds
		final TriggerModel trigger = modelService.create(TriggerModel.class);
		trigger.setCronExpression("0/10 * * * * ? *");
		trigger.setJob(job);
		trigger.setActive(Boolean.TRUE);
		job.setNodeGroup("test");
		modelService.save(trigger);

		final Calendar now = Calendar.getInstance(i18nService.getCurrentTimeZone(), i18nService.getCurrentLocale());
		((DefaultTriggerService) triggerService).activateForTest(trigger, now.getTimeInMillis());

		assertThat(job.getCronJobs())
				.extracting(CronJobModel::getNodeGroup)
				.contains("test");
