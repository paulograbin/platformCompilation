...
import de.hybris.platform.jalo.flexiblesearch.hints.impl.HanaHints;
...
public class FlexibleSearchServiceTest extends ServicelayerTransactionalTest
{
	...
	public void testSearchWithSingleParams()
	{
		...
		final CatalogVersionModel hwVersion = flexibleSearchService
				.<CatalogVersionModel>search("SELECT {PK} FROM {CatalogVersion} WHERE {version}='Online' AND {catalog}=?catalog",
						Collections.singletonMap("catalog", hwCatalog))
				.getResult().get(0);
	...
	public void testComplexSearch2()
	{
		...
		final List jaloResultList = FlexibleSearch.getInstance()
		                                          .search(query, params, Arrays.asList(Product.class, String.class, String.class),
				                                          true, false, 0, -1).getResult();
	...
	public void testSearchRangeCombos()
	{
		...
		final de.hybris.platform.jalo.SearchResult<Integer> countSR = FlexibleSearch.getInstance()
		                                                                            .search("SELECT COUNT({pk}) FROM {Product}",
				                                                                            Integer.class);
	...
	{
		final String searchQuery = "GET {Item}";
	...
	public void testPLA8827_multiple()
	{
		...
		for (int index = 0; index < jaloResultList.size(); index++)
		{
			...
			assertEquals(jaloResultList.get(index).getCode(),
					((EnumerationValueModel) sr1.getResult().get(index).get(1)).getCode());
			assertTrue(sr2.getResult().get(index).get(1) instanceof EnumerationValueModel);
			assertEquals(jaloResultList.get(index).getCode(),
					((EnumerationValueModel) sr2.getResult().get(index).get(1)).getCode());
	...
	protected void randomizeStatuses(final ArticleApprovalStatus... statuses)
	{
		...
		{
			enumVals = Arrays.asList(ArticleApprovalStatus.APPROVED, ArticleApprovalStatus.CHECK,
	...
	public void testTranslateWithCustomSession()
	{
		...
		// mock session
		final MockSession mockSession = spy(new MockSession());

		// mock QueryPreprocessorEngine
		final QueryPreprocessorRegistry queryPreprocessorEngine = mock(QueryPreprocessorRegistry.class);
		when(((DefaultFlexibleSearchService) flexibleSearchService).getQueryPreprocessorRegistry())
				.thenReturn(queryPreprocessorEngine);

		// mock sessionService
		final SessionService mockSessionService = spy(sessionService);
		...
		{
			final List<CatalogVersionModel> cvModelList = searchQuery.getCatalogVersions() == null
					? new ArrayList<>()
	...
	@Test(expected = de.hybris.platform.servicelayer.exceptions.ModelNotFoundException.class)
	public void testSearchUniqueFailsWhenJaloObjectNoLongerValidExceptionThrownAndMappedToModelNotFoundException()
	{
		//given
		PK pkMock = mock(PK.class);

		SearchResult<Object> searchResultMock = mock(SearchResult.class);
		List<Object> resultModelListMock = mock(List.class);

		JaloObjectNoLongerValidException exception = new JaloObjectNoLongerValidException(pkMock,
				new RuntimeException("Cause msg"), "Message", 2);

		final FlexibleSearchQuery flexibleSearchQuery = mock(FlexibleSearchQuery.class);
		doReturn(searchResultMock).when(flexibleSearchService).search(flexibleSearchQuery);

		when(searchResultMock.getResult()).thenReturn(resultModelListMock);
		when(resultModelListMock.isEmpty()).thenReturn(false);
		when(resultModelListMock.size()).thenReturn(1);
		when(resultModelListMock.get(0)).thenThrow(exception);

		//when
		flexibleSearchService.searchUnique(flexibleSearchQuery);

		//then
		//JaloObjectNoLongerValidException is wrapped as ModelNotFoundException
	}

	@Test(expected = de.hybris.platform.servicelayer.exceptions.ModelNotFoundException.class)
	public void testSearchUniqueFailsWhenJaloCachedComputationExceptionThrownAndMappedToModelNotFoundException()
	{
		//given
		final SearchResult<Object> searchResultMock = mock(SearchResult.class);
		final List<Object> resultModelListMock = mock(List.class);
		final FlexibleSearchQuery flexibleSearchQuery = mock(FlexibleSearchQuery.class);

		//Constructor of Item.JaloCachedComputationException is protected we need to use subtype for test
		final Item.JaloCachedComputationException exception = new TestJaloCachedComputationException(new RuntimeException());
		...
		//then
		//JaloCachedComputationException is wrapped as ModelNotFoundException
	}
	...
	public void testFindByApprovalStatus()
	{
		...
		final FlexibleSearchQuery query = new FlexibleSearchQuery(
				"SELECT {pk} FROM {Product} where {" + ProductModel.APPROVALSTATUS + "} = ?status");
	...
	public void shouldRespectSearchRestrictionsInTranslateMethod() throws Exception
	{
		...
		createRestriction(principal.getGroups().iterator().next(), composedTypeModel, "FooBarInActive", "foo != 'bar'",
				Boolean.TRUE);
	...
	{
		// given
		final FlexibleSearchQuery fq = new FlexibleSearchQuery("SELECT {PK} FROM {User}");
		fq.addHints(JdbcHints.preparedStatementHints().withFetchSize(55));

		// when
		final SearchResult<UserModel> searchResult = flexibleSearchService.search(fq);
	...
	{
		// given
		Assume.assumeTrue("Current DB is not HANA, test ignored", Config.isHanaUsed());
		final QueryHint hint = HanaHints.create("NO_USE_OLAP_PLAN", "IGNORE_PLAN_CACHE");
		final FlexibleSearchQuery fq = new FlexibleSearchQuery("SELECT {PK} FROM {User}");
		fq.addHints(hint);
	...
	static class TestJaloCachedComputationException extends Item.JaloCachedComputationException
	{
		public TestJaloCachedComputationException(final Throwable cause)
		{
			super(cause);
		}
	}
