...
public class CleanupCronJobIntegrationTest extends ServicelayerTransactionalTest
{
	...
	{
		final int currentExistingAllCJs = countExistingCronJobs(null, null);
		final int currentExistingSuccCJs = countExistingCronJobs(CronJobResult.SUCCESS, null);
		final int currentExistingAbortedCJs = countExistingCronJobs(null, CronJobStatus.ABORTED);
		final int currentExistingFailureCJs = countExistingCronJobs(CronJobResult.FAILURE, CronJobStatus.UNKNOWN);
		...
		assertEquals(currentExistingAllCJs + 31, countExistingCronJobs(null, null));
		assertEquals(currentExistingSuccCJs + 3, countExistingCronJobs(CronJobResult.SUCCESS, null));
		assertEquals(currentExistingAbortedCJs + 10, countExistingCronJobs(null, CronJobStatus.ABORTED));
		assertEquals(currentExistingFailureCJs + 16, countExistingCronJobs(CronJobResult.FAILURE, CronJobStatus.UNKNOWN));
		...
		assertEquals(CronJobResult.SUCCESS, result.getResult());
		assertEquals(CronJobStatus.FINISHED, result.getStatus());
		assertEquals(currentExistingAllCJs + 3, countExistingCronJobs(null, null));
	}
	...
	public void testCleanupCronjob_ExclusionList() throws ImpExException
	{
		...
		assertEquals(currentExistingCJs + 2, countExistingCronJobs(CronJobResult.SUCCESS, CronJobStatus.FINISHED));
	...
	{
		final int currentExistingCJs = countExistingCronJobs(CronJobResult.SUCCESS, CronJobStatus.FINISHED);
		createCronJobs("cleanupcronjobs", CronJobResult.SUCCESS, CronJobStatus.FINISHED, 5, 5,
		               "CleanupCronJob");
		...
		createCronJobs("normaljobs", CronJobResult.SUCCESS, CronJobStatus.FINISHED, 5, 5);
		the_job.setSearchType(typeService.getComposedTypeForCode("CronJob"));
		the_job.setThreshold(null);
		result = gmjp.perform(cjm);
	...
	public void testOverrideBeanValuesByJobCronjobWithChangingSearchType() throws ImpExException
	{
		...
		createCronJobs("normaljob", CronJobResult.SUCCESS, CronJobStatus.FINISHED, 5, 5);
		the_job.setSearchType(null);
		result = gmjp.perform(cjm);
		assertEquals(CronJobResult.SUCCESS, result.getResult());
		assertEquals(CronJobStatus.FINISHED, result.getStatus());
		assertEquals(currentExistingCJs, countExistingCronJobs(CronJobResult.SUCCESS, CronJobStatus.FINISHED));
	                            ...
	                            final int daysold, final String cronjobtype) throws ImpExException
	{
		...
		for (int index = 0; index < count; index++)
		{
			...
			cronjobbuffer.append(
					cjr.getCode() + ";" + start.getDayOfMonth() + "-" + start.getMonthOfYear() + "-" + start.getYear());
