	                                   ...
	                                   final Collection<CatalogVersionModel> versions)
	{
		if (versions == null || versions.isEmpty())
		{
			...
			if (LOG.isDebugEnabled())
			{
				...
				LOG.debug("No active versions available for request " + Sanitizer.sanitize(reqURI) + "!");
			}
		}
		else
		{
		...
		for (final Iterator it = ret.iterator(); it.hasNext(); )
		{
			...
			if (patterns != null && !patterns.isEmpty())
			{
				...
				if (LOG.isDebugEnabled())
				{
					...
					{
						LOG.debug(
								"Request " + Sanitizer.sanitize(requestStr) + " matched url patterns " + patterns + ". Added version " + catalogVersion
										+ ".");
					}
					else
					{
						LOG.debug("Request " + Sanitizer.sanitize(requestStr) + " did not match url patterns " + patterns + ". Filtered version "
								+ catalogVersion + ".");
					}
				}
			}
