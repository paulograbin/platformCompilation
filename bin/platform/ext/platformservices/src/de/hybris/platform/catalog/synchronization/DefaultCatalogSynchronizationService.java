...
public class DefaultCatalogSynchronizationService implements CatalogSynchronizationService
{
	...
	public SyncResult synchronize(final SyncItemJobModel syncJob, final SyncConfig syncConfig)
	{
		...
		if (Boolean.FALSE.equals(!syncConfig.hasPartialSyncSchedule()))
		{
		...
		if (!isSynchronous(syncConfig) && isClusterSpecific(syncCronJob))
		{
			cronJobService.performCronJob(syncCronJob);
		}
		else
		{
			cronJobService.performCronJob(syncCronJob, isSynchronous(syncConfig));
		}
		return syncResult;
	}

	private boolean isSynchronous(final SyncConfig syncConfig)
	{
		return syncConfig.getSynchronous() == null || syncConfig.getSynchronous();
	}

	private boolean isClusterSpecific(final CronJobModel cronJob)
	{
		return cronJob.getNodeID() != null || cronJob.getNodeGroup() != null;
	}
	                                             ...
	                                             final List<ItemModel> applicableItems)
	{
		...
		for (final ItemModel theItem : applicableItems)
		{
			// item is from source
			if (ObjectUtils
					.equals(syncJob.getSourceVersion(), catalogTypeService.getCatalogVersionForCatalogVersionAwareModel(theItem)))
			{
	...
	/**
	 * {@inheritDoc}
	 * Returns true if the principal has sync permissions by checking sync job has syncPrincipalsOnly set to true and
	 * given principal is part of sync job's syncPrincipals list or by checking write permissions on target catalog.
	 */
	@Override
	public boolean canSynchronize(final SyncItemJobModel syncJob, final PrincipalModel principal)
	{
		ServicesUtil.validateParameterNotNull(syncJob, "syncJob cannot be null");
		ServicesUtil.validateParameterNotNull(principal, "principal cannot be null");

		if (principal instanceof UserModel && userService.isAdmin((UserModel) principal))
		{
			return true;
		}

		if (principal instanceof UserGroupModel && userService.isAdminGroup((UserGroupModel) principal))
		{
			return true;
		}

		if (Boolean.TRUE.equals(syncJob.getSyncPrincipalsOnly()))
		{
			final Set<PrincipalModel> allGroups = Sets.newHashSet(principal.getAllGroups());
			allGroups.add(principal);

			final List<PrincipalModel> syncPrincipals = syncJob.getSyncPrincipals();

			return isNotEmpty(syncPrincipals) && isNotEmpty(intersection(allGroups, syncPrincipals));
		}
		else
		{
			final CatalogVersionModel targetVersion = syncJob.getTargetVersion();
			return catalogVersionService.getAllWritableCatalogVersions(principal).contains(targetVersion);
		}
	}
	...
	protected SyncItemCronJobModel createSyncCronJob(final SyncItemJobModel job)
	{
		...
		cronJob.setNodeID(job.getNodeID());
		cronJob.setNodeGroup(job.getNodeGroup());

		if (job.getUseReadOnlyDatasource() != null)
		{
			cronJob.setUseReadOnlyDatasource(job.getUseReadOnlyDatasource());
		}
