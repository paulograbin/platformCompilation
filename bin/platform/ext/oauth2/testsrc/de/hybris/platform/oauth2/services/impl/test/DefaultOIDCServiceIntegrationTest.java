...
import de.hybris.platform.oauth2.jwt.util.RsaKeyStoreHelperTest;
...
public class DefaultOIDCServiceIntegrationTest extends ServicelayerTransactionalTest
{
	...
	public void setUp() throws IOException
	{
		...
		try (final InputStream inputStream = DefaultOIDCServiceIntegrationTest.class.getResourceAsStream(KEY_STORE_PROPERTIES_LOCATION)) {
	...
	public void testConfiguration()
	{
		...
		assertThat(configuration.get(OAuth2Constants.OID_RESPONSE_TYPES)).isEqualTo(
		...
		assertThat(configuration.get(OAuth2Constants.OID_TOKEN_ENDPOINT_AUTH_METHODS)).isEqualTo(
				Arrays.asList("client_secret_post", "client_secret_basic"));
		assertThat(configuration.get(OAuth2Constants.OID_JWKS_URI)).isEqualTo(
				"https://myserver.sap.corp:9002/authorizationserver/.well-known/jwks.json");
		assertThat(configuration.get(OAuth2Constants.OID_TOKEN_ENDPOINT)).isEqualTo(
				"https://myserver.sap.corp:9002/authorizationserver/oauth/token");
		assertThat(configuration.get(OAuth2Constants.OID_AUTH_ENDPOINT)).isEqualTo(
				"https://myserver.sap.corp:9002/authorizationserver/oauth/authorize");
	...
	public void testJWKS()
	{
		...
		assertThat(jwks.get("keys").get(0).get("n")).isEqualTo(
				org.apache.commons.codec.binary.Base64.encodeBase64URLSafeString(publicKey.getModulus().toByteArray()));
		assertThat(jwks.get("keys").get(0).get("e")).isEqualTo(
				org.apache.commons.codec.binary.Base64.encodeBase64URLSafeString(publicKey.getPublicExponent().toByteArray()));
	}
