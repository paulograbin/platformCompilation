...
import de.hybris.platform.validation.internal.xml.binding.BeanType;
import de.hybris.platform.validation.internal.xml.binding.ConstraintType;
import de.hybris.platform.validation.internal.xml.binding.ElementType;
import de.hybris.platform.validation.internal.xml.binding.FieldType;
import de.hybris.platform.validation.internal.xml.binding.GetterType;
import de.hybris.platform.validation.services.impl.DefaultInterfaceGenerator;
import de.hybris.platform.validation.services.impl.DefaultLanguagePayloadService;
...
public class AttributeConstraintConverterTest
{
	...
	public void setUp()
	{
		...
		builder.setLanguagePayloadService(new DefaultLanguagePayloadService(new DefaultInterfaceGenerator()));
	}
	...
	public void testSimplePojoConstraintParse()
	{
		...
		((ItemModelContextImpl) ModelContextUtils.getItemModelContext(patternConstraint)).setLocaleProvider(
				new StubLocaleProvider(
						Locale.ENGLISH));
	...
	@Test
	public void testAddingLanguagePayloadToLocalizedAttributeConstraint()
	{
		final NotBlankConstraintModel attributeOne = new NotBlankConstraintModel();
		attributeOne.setId("one");
		attributeOne.setActive(true);
		attributeOne.setAnnotation(NotBlank.class);
		attributeOne.setTarget(PojoTwo.class);
		attributeOne.setQualifier("pojoOnePrivate");
		attributeOne.setConstraintGroups(Collections.emptySet());
		final LanguageModel languageModel = languageMock("languageId");
		attributeOne.setLanguages(Set.of(languageModel));

		checkLocalizedConstraintLanguage(builder.convert(attributeOne), languageModel);
	}

	private static LanguageModel languageMock(final String pk)
	{
		final LanguageModel languageModel = mock(LanguageModel.class);
		final PK pkMock = mock(PK.class);
		when(pkMock.toString()).thenReturn(pk);
		when(languageModel.getPk()).thenReturn(pkMock);
	...
	private void checkSingleModelHierarchyPojoConstraint(final BeanType bean)
	{
		...
		assertTrue("Should not create field since description is not accessible directly from PojoClass class ", bean.getField()
		                                                                                                             .size() == 0);
		assertTrue("Should create getter for description is only accessible as inhereited method  ",
				bean.getGetter().size() == 1);
		final GetterType getter = bean.getGetter().get(0);
		assertEquals("pojoOnePrivate", getter.getName());
		assertEquals(Boolean.FALSE, getter.getIgnoreAnnotations());
	...
	private void checkLocalizedConstraintLanguage(final BeanType bean, final LanguageModel languageModel)
	{
		assertEquals(1, bean.getGetter().size());

		final List<ConstraintType> constraints = bean.getGetter().get(0).getConstraint();
		assertEquals(1, constraints.size());

		final List<String> payloads = constraints.get(0).getPayload().getValue();
		assertEquals(1, payloads.size());

		final String payload = payloads.get(0);
		final String expectedLanguagePayloadName =
				ValidationConstants.LANGUAGE_PAYLOAD_INTERFACE_NAME_PREFIX + languageModel.getPk().toString();
		assertEquals(expectedLanguagePayloadName, payload);
	}
}
