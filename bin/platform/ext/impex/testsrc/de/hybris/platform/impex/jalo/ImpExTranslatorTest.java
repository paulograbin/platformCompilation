...
import de.hybris.platform.persistence.security.SpringPasswordEncoder;
...
public class ImpExTranslatorTest extends AbstractImpExTest
{
	...
	public void testConvertPlaintextToEncodedUserPasswordTranslator()
	{
		final String line = "INSERT_UPDATE Employee;" + Employee.UID + "[unique=true];"
				...
				+ "; foo ; bcrypt:foobar ; Foo";

		PasswordEncoder enc = null;
		try
		{
			enc = Registry.getCurrentTenant().getJaloConnection().getPasswordEncoder("bcrypt");
		}
		catch (final PasswordEncoderNotFoundException e)
		{
			fail("cannot get bcrypt password encoder (though it is mapped) : " + e);
		}
		...
		try
		{
			...
			if (enc instanceof SpringPasswordEncoder)
			{
				final String translatedPassword = foo.getEncodedPassword();
				assertTrue(enc.check(foo.getUid(), translatedPassword, "foobar"));
			}
			else
			{
				fail("Unsupported bcrypt encoder configured");
			}
			try
			{
				foo.getPassword();
			...
			assertEquals("bcrypt", foo.getPasswordEncoding());
		}
		catch (final Exception e)
		{
			fail(e.getMessage());
	...
	@Test
	public void testConvertPlaintextToEncodedUserPasswordTranslatorForDeprecatedEncoding() throws ImpExException
	{
		// given
		propertyConfigSwitcher.switchToValue(PasswordEncoderTestUtils.LEGACY_PASSWORD_ENCODING_ENABLED, "true");
		propertyConfigSwitcher.switchToValue(PasswordEncoderTestUtils.DEFAULT_PASSWORD_ENCODING, "bcrypt");

		final String password = org.apache.commons.lang3.RandomStringUtils.randomAlphanumeric(3);
		final String line = "INSERT_UPDATE Employee;" + Employee.UID + "[unique=true];"
				+ ImpExConstants.Syntax.SPECIAL_COLUMN_PREFIX + "password[" + ImpExConstants.Syntax.Modifier.TRANSLATOR + "="
				+ ConvertPlaintextToEncodedUserPasswordTranslator.class.getName() + "];" + Employee.NAME + "\n"
				+ "; foo ; md5:" + password + " ; Foo";

		final PasswordEncoder enc = Registry.getCurrentTenant().getJaloConnection().getPasswordEncoder("bcrypt");
		final ImpExImportReader impExImportReader = new ImpExImportReader(line);

		// when
		final Employee foo = (Employee) impExImportReader.readLine();

		// then
		assertEquals("foo", foo.getUID());
		assertTrue(enc.check(foo.getUid(), foo.getEncodedPassword(), password));
	}
	...
	public void testUserPasswordTranslatorImport()
	{
		...
		final String user1Password = RandomStringUtils.randomAlphanumeric(16);
		final String user2Password = RandomStringUtils.randomAlphanumeric(16);
		final String user1PasswordEncoded = enc.encode(userID1, user1Password);

		final String lines = "INSERT_UPDATE Employee;" + Employee.UID + "[unique=true];" + ImpExConstants.Syntax.SPECIAL_COLUMN_PREFIX
				+ "password[" + ImpExConstants.Syntax.Modifier.TRANSLATOR + "=" + UserPasswordTranslator.class.getName() + "];"
				+ Employee.NAME + "\n" + "# empty line \n" + ";" + userID1 + "; bcrypt:" + user1PasswordEncoded + ";" + username1 + "\n"
				+ ";" + userID2 + "; *:" + user2Password + ";" + username2 + "\n";
		...
		try
		{
			employee1 = (Employee) impExImportReader.readLine();
			assertEquals(userID1, employee1.getLogin());
			assertEquals(userID1, employee1.getUID());
			assertEquals(username1, employee1.getName());

			assertEquals(user1PasswordEncoded, employee1.getEncodedPassword());
			...
			assertEquals("bcrypt", employee1.getPasswordEncoding());

			employee2 = (Employee) impExImportReader.readLine();

			final PasswordEncoder encoder2 = Registry.getCurrentTenant().getJaloConnection()
			                                         .getPasswordEncoder(employee2.getPasswordEncoding());

			assertEquals(userID2, employee2.getLogin());
			assertEquals(userID2, employee2.getUID());
			assertEquals(username2, employee2.getName());
			assertTrue(encoder2.check(employee2.getUid(), employee2.getEncodedPassword(), user2Password));

			assertNotEquals(SystemEJB.DEFAULT_ENCODING, employee2.getPasswordEncoding());
	...
	public void testUserPasswordTranslatorExport()
	{
		...
		Employee employee1 = null, employee2 = null, empty = null;
		final String defaultPassword1 = RandomStringUtils.randomAlphanumeric(16);
		final String defaultPassword2 = RandomStringUtils.randomAlphanumeric(16);

		try
		{
			employee1 = UserManager.getInstance().createEmployee("axel");
			employee1.setName("Axel");
			employee1.setPassword(defaultPassword1, "bcrypt");
		}
		catch (final ConsistencyCheckException e)
		{
			fail(e.getMessage());
		...
		assertEquals("axel", employee1.getLogin());
		assertEquals("axel", employee1.getUID());
		assertEquals("Axel", employee1.getName());
		...
		final String defaultPasswordHash1 = enc.encode("axel", defaultPassword1);

		assertTrue(enc.check("axel", defaultPasswordHash1, defaultPassword1));
		...
		assertEquals("bcrypt", employee1.getPasswordEncoding());
		assertTrue(employee1.checkPassword(defaultPassword1));
		...
		try
		{
			employee2 = UserManager.getInstance().createEmployee("axel2");
			employee2.setName("noch einer");
			employee2.setPassword(defaultPassword2);
		}
		catch (final ConsistencyCheckException e)
		{
			fail(e.getMessage());
		...
		assertEquals("axel2", employee2.getLogin());
		assertEquals("axel2", employee2.getUID());
		assertEquals("noch einer", employee2.getName());
		assertEquals(Config.getString("default.password.encoding", SystemEJB.DEFAULT_ENCODING), employee2.getPasswordEncoding());
		...
		try
		{
			...
			impExExportWriter.writeLine(employee1);
			impExExportWriter.writeLine(employee2);
		...
		final String expected = header + CSVConstants.DEFAULT_LINE_SEPARATOR + ";" + employee1.getUID() + ";"
				+ employee1.getPasswordEncoding() + ":" + employee1.getEncodedPassword() + ";" + employee1.getName()
				+ CSVConstants.DEFAULT_LINE_SEPARATOR + ";" + employee2.getUID() + ";" + employee2.getPasswordEncoding() + ":"
				+ employee2.getEncodedPassword() + ";" + employee2.getName() + CSVConstants.DEFAULT_LINE_SEPARATOR + ";empty;"
	...
	public void testNestedDateFormatExport()
	{
		...
		assertThat(itemExpressionTranslator.exportValue(media)).isEqualTo(
				format.format(container.getCreationTime()).replace(":", "\\:"));
	}
