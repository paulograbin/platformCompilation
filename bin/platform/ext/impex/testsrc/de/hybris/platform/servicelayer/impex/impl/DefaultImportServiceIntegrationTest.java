...
public class DefaultImportServiceIntegrationTest extends ServicelayerTest
{
	...
	private final PropertyConfigSwitcher removeOnSuccessSwitcher = new PropertyConfigSwitcher(
			DefaultImportService.DISTRIBUTED_IMPEX_SYNCHRONOUS_REMOVEONSUCCESS_ENABLED);
	...
	public void setLegacyMode()
	{
		...
		removeOnSuccessSwitcher.switchBackToDefault();
	...
	public void testFileBasedImportResource() throws IOException
	{
		...
		final File testFile = Files.createTempFile("test", "test").toFile();
	...
	{
		// given
		removeOnSuccessSwitcher.switchToValue("true");
		removeOnSuccessImportBatchContentSwitcher.switchToValue("false");
		final Map<String, Integer> countInfoBefore = countStaleItemsForProcess();
		...
		assertThat(importResult.isFinished()).isTrue();
		assertThat(errors).isEmpty();
		assertThatImportedDataAreCorrect();
		final Map<String, Integer> countInfoAfter = countStaleItemsForProcess();
		assertThatStaleItemsAreRemoved(countInfoBefore, countInfoAfter);
	}
	...
	private void assertThatStaleItemsAreRemoved(final Map<String, Integer> countInfoBefore,
	                                            final Map<String, Integer> countInfoAfter)
	{
		assertThat(getDifferenceForCounter(countInfoBefore, countInfoAfter, "ImportBatch")).isZero();
		assertThat(getDifferenceForCounter(countInfoBefore, countInfoAfter, "ImportBatchContent")).isZero();
		assertThat(getDifferenceForCounter(countInfoBefore, countInfoAfter, "DistributedImportProcess")).isZero();
		assertThat(getDifferenceForCounter(countInfoBefore, countInfoAfter, "ImpExImportCronJob")).isZero();
	}
	...
	{
		// given
		removeOnSuccessSwitcher.switchToValue("false");
		removeOnSuccessImportBatchContentSwitcher.switchToValue("false");
		final Map<String, Integer> countInfoBefore = countStaleItemsForProcess();
		final ImportConfig config = prepareImportConfig();
		...
		final Map<String, Integer> countInfoAfter = countStaleItemsForProcess();
		assertThatStaleItemsAreNotRemoved(countInfoBefore, countInfoAfter);
	}

	private void assertThatStaleItemsAreNotRemoved(final Map<String, Integer> countInfoBefore,
	                                               final Map<String, Integer> countInfoAfter)
	{
		assertThat(getDifferenceForCounter(countInfoBefore, countInfoAfter, "ImportBatch")).isPositive();
		assertThat(getDifferenceForCounter(countInfoBefore, countInfoAfter, "ImportBatchContent")).isPositive();
		assertThat(getDifferenceForCounter(countInfoBefore, countInfoAfter, "DistributedImportProcess")).isPositive();
		assertThat(getDifferenceForCounter(countInfoBefore, countInfoAfter, "ImpExImportCronJob")).isPositive();
	}
	...
	private void assertThatImportedDataAreCorrect()
	{
		assertThat(findAddressByFirstNameAndLastName("foo1", "bar1")).isNotNull();
		assertThat(findAddressByFirstNameAndLastName("foo2", "bar2")).isNotNull();
		assertThat(findAddressByFirstNameAndLastName("foo3", "bar3")).isNotNull();
	...
	private int getDifferenceForCounter(final Map<String, Integer> before, final Map<String, Integer> after, final String cntName)
	{
		return after.get(cntName) - before.get(cntName);
	}

	private Map<String, Integer> countStaleItemsForProcess()
	{
		final Map<String, Integer> countInfo = new HashMap<>();
		countInfo.put("ImportBatch", countItemsForQuery("SELECT COUNT( {" + Item.PK + "})  FROM {ImportBatch}"));
		countInfo.put("ImportBatchContent", countItemsForQuery("SELECT COUNT( {" + Item.PK + "})  FROM {ImportBatchContent}"));
		countInfo.put("DistributedImportProcess", countItemsForQuery("SELECT COUNT( {" + Item.PK + "})  FROM {DistributedImportProcess}"));
		countInfo.put("ImpExImportCronJob", countItemsForQuery("SELECT COUNT( {" + Item.PK + "})  FROM {ImpExImportCronJob}"));
		return countInfo;
	}

	private int countItemsForQuery(final String cntQuery)
	{
		final FlexibleSearchQuery fQuery = new FlexibleSearchQuery(cntQuery);
	...
	public void shouldDumpLinesToLogFileWithInvalidOrNoHeaderWhenMultiThreadedImport() throws InterruptedException, IOException
	{
		...
		// when
		final ImportResult importResult = importService.importData(config);
		long maxWait = System.currentTimeMillis() + (30 * 1000);
		do
		{
			Thread.sleep(100);
		}
		while (!importResult.isFinished() && System.currentTimeMillis() < maxWait);
		maxWait = System.currentTimeMillis() + (30 * 1000);
		do
		{
			Thread.sleep(100);
		}
		while (importResult.getCronJob().getLogFiles().isEmpty() && System.currentTimeMillis() < maxWait);
		// then
		assertThat(importResult.hasUnresolvedLines()).isTrue();
		assertThat(config.getMaxThreads()).isGreaterThan(1);
		assertThat(config.isSynchronous()).isFalse();

		final Collection<LogFileModel> logFiles = importResult.getCronJob().getLogFiles();
		...
		try (final ZipInputStream zip = new ZipInputStream(new ByteArrayInputStream(rawZip)))
		{
			...
			assertThat(logFileData).contains("dumped unresolved line");
		}
	}
	...
	public void importConfigShouldOverrideFlagForDistributedImpexRemoveOnSuccessModeSetToNone()
	{
		...
		removeOnSuccessSwitcher.switchToValue("false");
		final ImportConfig importConfig = new ImportConfig();
		importConfig.setRemoveOnSuccessForDistributedImpex(true);

		// when
		final boolean distributedImpexEnabled = ((DefaultImportService) importService).isRemoveOnSuccessForDistributedImpexEnabled(
				importConfig);
	...
	public void flagRemoveOnSuccessModeShouldSetShouldBeRespectedIfImportConfigIsNotSet()
	{
		...
		// then
		assertThat(importConfig.isDistributedImpexEnabled()).isNull();
		assertThat(distributedImpexEnabled).isTrue();
	}
