...
public class ImpExManagerTest extends AbstractImpExTest
{
	...
	public void testImportDataLightReader()
	{
		try
		{
			final InputStream inputStream = ImpExManager.class.getResourceAsStream("/impex/testfiles/productFull.impex");
			assertNotNull("Can not read from jar file 'productFull.impex'", inputStream);
			final Importer importer = ImpExManager.getInstance()
			                                      .importDataLight(new CSVReader(inputStream, windows1252.getCode()), false, 1);
			assertNotNull(importer);
			assertFalse(importer.hadError());
		}
	...
	public void testImportDataFull()
	{
		...
		final ImpExImportCronJob cronJob = ImpExManager.getInstance().importData(inputStream, windows1252.getCode(),
				CSVConstants.HYBRIS_FIELD_SEPARATOR,
	...
	public void testImportDataUnresolved()
	{
		...
		try
		{
			TestUtils.disableFileAnalyzer("the import results in one not imported line - this is ok, needed for the test here",
					100);
			cronJob = ImpExManager.getInstance()
			                      .importData(inputStream, windows1252.getCode(), CSVConstants.HYBRIS_FIELD_SEPARATOR,
					                      CSVConstants.HYBRIS_QUOTE_CHARACTER, false);
		}
		finally
		{
	...
	public void testImportDataLightNested()
	{
		try
		{
			...
			final Importer importer = ImpExManager.getInstance()
			                                      .importDataLight(new CSVReader(inputStream, windows1252.getCode()), true, 1);
			assertNotNull(importer);
			assertFalse(importer.hadError());
		}
		catch (final UnsupportedEncodingException e1)
	...
	public void testImportDataAndMedia()
	{
		...
		try
		{
			...
			final ImpExMedia jobMedia = ImpExManager.getInstance()
			                                        .createImpExMedia("generated impex media - " + UUID.randomUUID(),
					                                        windows1252.getCode());
	...
	public void testExportImport()
	{
		...
		try
		{
			...
			scriptFile = Files.createTempFile("test_importexport_exportscript", ".temp").toFile();
