...
public class DefaultMediaStorageConfigServiceTest
{
	...
	{
		final Map<String, ConfigValueConverter> result = new HashMap<>();
		...
		result.put("mediaFolderName.validation", new BooleanValueConverter());
		result.put("local.cache.blockSize", new IntegerValueConverter());
		return result;
	}
	...
	public void shouldReturnConfigForNotDirectlyConfiguredFolder()
	{
		...
		assertThat(config.getParameter(localCacheBlockSizeKey, Integer.class)).isNull();
	}
	...
	public void shouldBuildConfigForFolder()
	{
		...
		assertThat(config.getParameter("accessKeyId")).isEqualTo("123456");
		final String localCacheBlockSizeKey = DefaultSettingKeys.LOCAL_CACHE_BLOCK_SIZE.getKey();
		assertThat(config.getParameter(localCacheBlockSizeKey, Integer.class)).isEqualTo(1234);
	}
	...
	private Map<String, String> getDefaults()
	{
		...
		result.put("media.default.local.cache.blockSize", "4096");
		...
		result.put("media.default.mediaFolderName.validation", "true");
	...
	private Map<String, String> getFoldersSettings()
	{
		...
		result.put("media.folder.folderOther.local.cache.blockSize", "1234");
