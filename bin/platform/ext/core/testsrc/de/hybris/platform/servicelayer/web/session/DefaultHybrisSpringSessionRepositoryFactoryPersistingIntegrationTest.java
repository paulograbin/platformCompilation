...
public class DefaultHybrisSpringSessionRepositoryFactoryPersistingIntegrationTest extends ServicelayerBaseTest
{
	...
	@Test
	public void persistSessionWithContextQueryFilter() throws InterruptedException
	{
		SESSION_SAVE_TYPE.switchToValue(SYNC);
		SESSION_FILTERING_STRATEGY.switchToValue(ANONYMOUS_STRATEGY);

		final PersistedSession hacSession = createHacSession();

		final ComposedTypeModel ctm = typeService.getComposedTypeForClass(LanguageModel.class);
		final ComposedType ct = modelService.getSource(ctm);
		final ContextQueryFilter filter = new ContextQueryFilter("pl", ct, "{active} IS TRUE");
		final Collection<ContextQueryFilter> contextQueryFilters = new ArrayList<>();
		contextQueryFilters.add(filter);

		hacSession.setAttribute("ctxSearchRestrictions", contextQueryFilters);

		createRepository().save(hacSession);
