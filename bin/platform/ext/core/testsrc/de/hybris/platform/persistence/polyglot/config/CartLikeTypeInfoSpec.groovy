...
@UnitTest
class CartLikeTypeInfoSpec extends HybrisSpockSpecification {
    ...
    @Test
    def testIsKeyUsedInConfig(int typeCode, String qualifier, boolean expectedResult) {
        when:
        final boolean result = RepositoryConfigUtils.isKeyUsedInConfig(factory, TypeInfoFactory.getTypeInfo(typeCode), qualifier);

        then:
        result == expectedResult

        where:
        typeCode | qualifier | expectedResult
        1 | "pk" | true
        1 | "c" | true
        1 | "d" | false
        2 | "c" | true
