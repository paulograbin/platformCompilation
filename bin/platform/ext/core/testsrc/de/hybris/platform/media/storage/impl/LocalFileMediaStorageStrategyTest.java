...
public class LocalFileMediaStorageStrategyTest extends ServicelayerBaseTest
{
	...
	private static final String TEMP_MEDIA_DATA_DIR ="tempMediaData";
	...
	{
		final String tempDir = Files.createDirectories(Paths.get(createTempDirectoryOnPlatformPath()
			+File.separator+TEMP_MEDIA_DATA_DIR)).toString();
		...
		finally
		{
			...
			FileUtils.deleteQuietly(Paths.get(tempDir).toFile());
		}
	}

	private String createTempDirectoryOnPlatformPath()
	{
		final PlatformConfig platformConfig = ConfigUtil.getPlatformConfig(LocalFileMediaStorageStrategyTest.class);
	...
	public void shouldNotGetMediaFromTamperedLocationAsStream() throws Exception
	{
		...
		try
		{
			...
			// when
			mediaAsStream = mediaStorageStrategy.getAsStream(storageConfigService.getConfigForFolder("bar"),
					storeMedia.getLocation());
	...
	public void shouldNotGetMediaFromTamperedLocationAsRootFolderAsStream() throws Exception
	{
		...
		try
		{
			...
			// when
			mediaAsStream = mediaStorageStrategy.getAsStream(
					storageConfigService.getConfigForFolder(MediaManager.ROOT_FOLDER_QUALIFIER), storeMedia.getLocation());
