...
public class TypeManagerTest extends HybrisJUnit4TransactionalTest
{
	...
	@Test
	public void testPrepareItemRemove_shouldRemoveRelationDescriptors() throws Exception
	{
		// given
		// UserType as 'members' ---{userUserGroupTestRelation}--> UserGroupType as 'groups'
		final String relationQualifer = "userUserGroupTestRelation";
		...
		final AttributeDescriptorRemote sourceFeatureToBeRemoved = (AttributeDescriptorRemote) EJBTools.instantiateItemPropertyValue(
				(ItemPropertyValue) (relationType).getProperty(RelationType.SOURCE_ATTRIBUTE));
		final AttributeDescriptorRemote targetFeatureToBeRemoved = (AttributeDescriptorRemote) EJBTools.instantiateItemPropertyValue(
				(ItemPropertyValue) (relationType).getProperty(RelationType.TARGET_ATTRIBUTE));
		assertNotNull(sourceFeatureToBeRemoved);
		assertNotNull(targetFeatureToBeRemoved);

		// when
		typeManager.prepareItemRemove(relationType);

		// then
		final AttributeDescriptorRemote removedSourceFeature = (AttributeDescriptorRemote) EJBTools.instantiateItemPropertyValue(
				(ItemPropertyValue) (relationType).getProperty(RelationType.SOURCE_ATTRIBUTE));
		final AttributeDescriptorRemote removedTargetFeature = (AttributeDescriptorRemote) EJBTools.instantiateItemPropertyValue(
				(ItemPropertyValue) (relationType).getProperty(RelationType.TARGET_ATTRIBUTE));
		assertNull(removedSourceFeature);
		assertNull(removedTargetFeature);
	}
