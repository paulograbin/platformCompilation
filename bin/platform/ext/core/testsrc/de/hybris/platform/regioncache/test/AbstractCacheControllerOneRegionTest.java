...
public abstract class AbstractCacheControllerOneRegionTest
{
	...
	{
		final TestThreadsHolder<Runnable> randomAccessHolder = new TestThreadsHolder<>(128,
						...
						return new Runnable()
						{
							...
							public void run()
							{
								try
								{
									...
									for (int i = 0; i < 990; ++i)
									{
										final CacheKey key = new TestCacheKey(
												"key" + i);
										controller.getWithLoader(
												key,
												loader); // , region.getName());
									}
								}
	...
	public void statsTest()
	{
		...
		final TestThreadsHolder<Runnable> randomAccessHolder = new TestThreadsHolder<>(8,
				new de.hybris.platform.test.RunnerCreator<Runnable>()
				{
							...
							return new Runnable()
							{
								...
								public void run()
								{
									Thread.currentThread()
									      .setName(
											      "statsTest-Thread-" + threadNumber);
									for (int i = 0; i < 200 * size; ++i)
									{
										final Integer k = Integer
												.valueOf(
														random.nextInt(
																size));
										...
										controller.remove(
												new TestCacheKey(
														k,
														CacheUnitValueType.NON_SERIALIZABLE,
														String.valueOf(
																k.intValue() / 10)));
									}
								}
	...
	public void statsTypesTest()
	{
		controller.getWithLoader(new LegacyCacheKey(new Object[]
						{ DefaultCache.CACHEKEY_C2LMANAGER, }, "master"),
				new TestCacheValueLoader("a"));

		final Collection<CacheRegion> regions = controller.getRegions();
		for (final CacheRegion region : regions)
		{
			final CacheStatistics stats = region.getCacheRegionStatistics();
			final List<Object> types = new LinkedList<>(stats.getTypes());
	...
	@Test
	public void maxReachedSizeTest()
	{
		...
		for (int i = 0; i < cacheSize; ++i)
		{
			final CacheKey key = new TestCacheKey("key" + i);
			controller.invalidate(key);
			Assert.assertEquals(cacheSize, region.getMaxReachedSize());
		}
