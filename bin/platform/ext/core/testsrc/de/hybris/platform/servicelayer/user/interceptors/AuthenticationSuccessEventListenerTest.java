...
public class AuthenticationSuccessEventListenerTest extends AbstractAuthenticationEventListenerTest
{
	...
	{
		createBruteForceLoginAttempts();
		listener.onApplicationEvent(new AuthenticationSuccessEvent(new UsernamePasswordAuthenticationToken(testUid, "pwd")));
		assertThat(findAttempts().getAttempts()).isZero();
	}
	...
	public void shouldNotResetFailedLoginAttemptsOnSuccessfulLoginWhenAuthenticationIsNotAllowedForEvent()
	{
		...
		//when
		listener.onApplicationEvent(new AuthenticationSuccessEvent(createNotAllowedAuthentication(testUid)));

		//then
		assertThat(findAttempts().getAttempts()).isEqualTo(1);
	}
	...
	{
		//given
		final String unknowUser = "unknowUser";
		createBruteForceLoginAttempts();

		//when
		listener.onApplicationEvent(new AuthenticationSuccessEvent(new UsernamePasswordAuthenticationToken(unknowUser, "pwd")));

		//then
		assertThat(findAttempts().getAttempts()).isEqualTo(1);
	}

	private void createBruteForceLoginAttempts()
	{
