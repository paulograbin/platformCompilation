...
public class MultiThreadedStaleDataTest
{
	...
	public void changesToCacheVisibleToOtherThread(final CacheValueFactory factory)
			throws InterruptedException, CacheRegionNotSpecifiedException
	{
		...
		createCacheValueAndInvalidateKey(0, factory);
					...
					return new Runnable()
					{
						...
						public void run()
						{
							...
							try
							{
								...
								{
									val = createCacheValueAndInvalidateKey(val, factory);
								}
							}
		...
		if (!workerThreads.waitForAll(30, TimeUnit.SECONDS))
		{
	...
	private int createCacheValueAndInvalidateKey(int val, final CacheValueFactory factory)
	{
