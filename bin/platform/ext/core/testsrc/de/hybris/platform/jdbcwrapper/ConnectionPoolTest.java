 ...
 * <br>
 * Some of the test cases in this class create connection pool having a maximum number of connections defined in db.pool.maxActive/db.pool.maxIdle properties.
 * Take that into consideration while running it locally for some locally run databases the limit for max connections may be reached. Consider increasing that limit if needed on the database side for local test execution.
 */
@IntegrationTest
public class ConnectionPoolTest extends HybrisJUnit4Test
{
		...
		private void simulateNormalConnection(final ConnectionImpl connection) throws SQLException
		{
			if (sendStatement)
			{
				...
				try
				{
					...
					stm.setString(1, runId + "/" + Thread.currentThread().getId());
					stm.executeQuery();
				}
				finally
				{
	...
	private static class TestJDBCConnectionPool extends JDBCConnectionPool
	{
		...
		@Override
		public ConnectionImpl borrowConnection() throws Exception
		{
