...
public class RemoteStorageMediaLoaderIntegrationTest extends ServicelayerBaseTest
{
	...
	private static final String LONG_FILENAME = RandomStringUtils.randomAlphanumeric(170);
	...
	private final PropertyConfigSwitcher preventLongfilenamesSwitcher = new PropertyConfigSwitcher(
			DefaultLocalMediaFileCacheService.PREVENT_LONGFILENAMES_LOCALCACHE);
	...
	private void cleanCacheMap(final boolean fireEvents)
		{
			...
			for (final CacheKey cacheKey : allKeys)
			{
				...
				{
				mediaCacheRegion.remove(cacheKey, fireEvents);
		}
		}
	}
	...
	public void shouldThrowExceptionWhenFilenameTooLong()
	{
		...
		given(dummyFileStrategy.store(any(), anyString(), any(), any()))
				.willReturn(new StoredMediaData(LONG_FILENAME, "hash", 1, IMAGE_JPEG));

		final InputStream sampleInputStream = getSampleInputStream(getRandomBytes());

		try
		{
			mediaService.setStreamForMedia(media3, sampleInputStream, LONG_FILENAME, IMAGE_JPEG);
