...
public class PolyglotQueryFlexibleSearchTest extends ServicelayerTransactionalBaseTest
{
	...
	public void testPolyglotQueryForAllTitles()
	{
		...
		final List<TitleModel> result = flexibleSearchService.<TitleModel>search(query).getResult();

		// then
		assertEquals(2, result.size());
		final List<String> resultCodes = result.stream().map(TitleModel::getCode).collect(Collectors.toList());
		assertTrue(resultCodes.contains("firstTitle"));
		assertTrue(resultCodes.contains("secondTitle"));
	}
	...
	@Test
	//@Ignore
	public void testPolyglotQueryWithAndAndOrOrderedBy()
	{
		// given
		final TitleModel title1 = new TitleModel();
		title1.setCode("firstTitle");
		...
		// when
		final String query = "GET {Title} WHERE {code}=?code1 OR {code}=?code2 AND {name[en]}=?name ORDER BY {code}";
		final Map<String, Object> params = new HashMap<>();
		params.put("code1", "firstTitle");
		params.put("code2", "secondTitle");
