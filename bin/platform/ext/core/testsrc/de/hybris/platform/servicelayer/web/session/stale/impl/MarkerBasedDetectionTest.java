...
public class MarkerBasedDetectionTest
{
	...
	public void shouldResetMarkerWhenPasswordChangedForTheSameUser()
	{
		...
		try (final MarkerBasedDetection detection = new MarkerBasedDetection(sessionContext);
		     final MockedStatic<Config> configMock = Mockito.mockStatic(Config.class))
		{
			configMock.when(() -> Config.getBoolean(INVALIDATE_CURRENT_AUTHENTICATION, false)).thenReturn(Boolean.FALSE);
			assertThat(detection.isStaleSession()).isFalse();
	...
	@Test
	public void shouldNotResetMarkerWhenPasswordChangedForTheSameUserAndInvalidationFlagIsTrue()
	{
		//given
		final SessionContext sessionContext = mockSessionContext("S_1", SOME_USER_ID, SOME_PASSWORD, SOME_PASSWORD_MARKER);
		...
		try (final MarkerBasedDetection detection = new MarkerBasedDetection(sessionContext);
		     final MockedStatic<Config> configMock = Mockito.mockStatic(Config.class))
		{
			configMock.when(() -> Config.getBoolean(INVALIDATE_CURRENT_AUTHENTICATION, false)).thenReturn(Boolean.TRUE);
			assertThat(detection.isStaleSession()).isFalse();
