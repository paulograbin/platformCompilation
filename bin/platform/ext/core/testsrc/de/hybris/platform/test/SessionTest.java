...
public class SessionTest extends HybrisJUnit4Test
{
	...
	public void testEvictedCartObjectNotReturned()
	{
		...
		final Cart cartObj0 = jaloSession.getCart();
		assertNotNull(cartObj0);
		assertAllSessionCartGetters(cartObj0);

		// clear cache --> cart object is evicted; getters should fetch *new* object
		Registry.getCurrentTenantNoFallback().getCache().clear();

		final Cart cartObj1 = jaloSession.getCart();
		assertNotNull(cartObj1);
		assertNotSame(cartObj0, cartObj1);
		assertAllSessionCartGetters(cartObj1);

		// clear cache once more --> cart object is evicted; getters should fetch *new* object
		Registry.getCurrentTenantNoFallback().getCache().clear();

		final Cart cartObj2 = jaloSession.getCart();
		assertNotNull(cartObj2);
		assertNotSame(cartObj1, cartObj2);
		assertNotSame(cartObj0, cartObj2);
		assertAllSessionCartGetters(cartObj2);
	}

	private void assertAllSessionCartGetters(final Cart cartDirect, final Cart... oldCarts)
	{
		...
		// is reference cart still in cache, or was it evicted (by background process, etc. )?
		final boolean cartObjStillInCache = cartDirect.isCacheBound();

		if (cartObjStillInCache)
		{
			// in cache --> others *must* be the same!!!
			assertSame(cartDirect, cartFromAttribute);
			assertSame(cartDirect, cartFromAttributes);
		}
		else
		{
			// not in cache (any more) --> other may still be the same, or not; at least must be equal
			if (cartDirect != cartFromAttributes)
			{
				assertEquals(cartDirect, cartFromAttributes);
			}
			if (cartDirect != cartFromAttribute)
			{
				assertEquals(cartDirect, cartFromAttribute);
			}
		}

		// make sure that attributes do never hold one of the old objects!!!
		if (oldCarts != null)
		{
			for (final Cart oldOne : oldCarts)
			{
				assertNotSame(oldOne, cartFromAttribute);
				assertNotSame(oldOne, cartFromAttributes);
			}
		}
	}
	...
	public void testDirectTransfer() throws ConsistencyCheckException, JaloInvalidParameterException, JaloSecurityException
	{
		...
		try
		{
			...
			props.put(LoginProperties.PASSWORD, firstPassword);
			jaloSession.transfer(props);

			props.put(LoginProperties.LOGIN, "foo");
			props.put(LoginProperties.PASSWORD, secondPassword);
			try
			{
				jaloSession.transfer(props);
	...
	// This test uses Transaction to make sure the password that is set for admin is not saved in the database.
	@Test
	public void performLoginAsAdminWithAdminLoginEnabledShouldSucceed() throws Exception
	{
		// given
		propertyConfigSwitcher.switchToValue(User.ADMIN_LOGIN_ENABLED, "true");

		final String temporaryAdminPassword = RandomStringUtils.randomAlphanumeric(16);
		final User adminUser = UserManager.getInstance().getAdminEmployee();
		final String adminPasswordBeforeTransaction = adminUser.getEncodedPassword();
		...
		try
		{
			Transaction.current().execute(() -> {
				// when
				adminUser.setPassword(temporaryAdminPassword, PasswordEncoderTestUtils.ARGON_2);
				jaloSession.transfer(UserConstants.ADMIN_EMPLOYEE_UID, temporaryAdminPassword);
				final User currentUser = jaloSession.getUser();

				// then
				assertThat(adminUser).isEqualTo(currentUser);

				// rollback to prevent password from being saved into the database
				Transaction.current().setRollbackOnly();
				return null;
			});
		}
		catch (final RollbackOnlyException e)
		{
			// ignored
		}

		assertThat(adminUser.getEncodedPassword()).isEqualTo(adminPasswordBeforeTransaction);
	}

	@Test
	public void performLoginAsAdminWithAdminLoginDisabledPasswordLoginShouldFail()
	{
		// given
		propertyConfigSwitcher.switchToValue(User.ADMIN_LOGIN_ENABLED, "false");

		// then
		assertThrows("Admin login is disabled", JaloSecurityException.class,
				() -> jaloSession.transfer(UserConstants.ADMIN_EMPLOYEE_UID, ""));
	}

	@Test
	public void performLoginAsAdminWithAdminLoginDisabledPropsShouldFail()
	{
		// given
		propertyConfigSwitcher.switchToValue(User.ADMIN_LOGIN_ENABLED, "false");

		final Map props = new HashMap();
		props.put(JaloSession.LoginProperties.LOGIN, UserConstants.ADMIN_EMPLOYEE_UID);
		props.put(JaloSession.LoginProperties.PASSWORD, "");

		// then
		assertThrows("Admin login is disabled", JaloSecurityException.class,
				() -> jaloSession.transfer(props, false));
	}
