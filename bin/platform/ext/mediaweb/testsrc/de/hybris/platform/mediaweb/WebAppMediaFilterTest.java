...
@IntegrationTest
public class WebAppMediaFilterTest extends ServicelayerBaseTest
{
	...
	@Test
	public void shouldServeMediaFileAsStreamWhenRequestingNormalResourceWithDecodedLocationInContext() throws Exception
	{
		// given
		given(httpRequest.getRequestURI()).willReturn(String.format("%s/medias/realFilename.jpg", CONTEXT_PATH));
		given(httpRequest.getContextPath()).willReturn(CONTEXT_PATH);
		given(httpRequest.getParameter("context"))
				.willReturn("bWFzdGVyfHJvb3R8MTIzNDV8aW1hZ2UvanBlZ3xoMDEvaDAyL2Zvby5qcGd8cXdlcnR5MTIzNDU"); //master|root|12345|image/jpeg|h01/h02/foo.jpg|qwerty12345
		setUpMocksBehaviorForStandardMediaResource();
	...
	@Test
	public void shouldServeMediaFileWithUnderscoresWhenRequestingXLSFileWithBadChars() throws Exception
	{
		// given
		given(httpRequest.getRequestURI()).willReturn(String.format("%s/medias/sys_master/real!#Filename.xls", CONTEXT_PATH));
		given(httpRequest.getContextPath()).willReturn(CONTEXT_PATH);
		setUpMocksBehaviorForStandardMediaResource();
		...
		// then
		verify(httpResponse, times(1)).addHeader("Content-Disposition", " attachment; filename=real__Filename.xls");
	}
