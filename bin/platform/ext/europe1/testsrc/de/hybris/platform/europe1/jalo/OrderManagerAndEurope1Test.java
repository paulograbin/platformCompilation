...
public class OrderManagerAndEurope1Test extends HybrisJUnit4Test
{
	...
	private final String[] addr1Fields = new String[]
			{ Address.FIRSTNAME, "Dirk", Address.LASTNAME, "Ahlrichs", Address.COMPANY, "Mainau GmbH", Address.STREETNAME, "Kriegstein 67",
					Address.TOWN, "Koeln", Address.POSTALCODE, "50400", Address.EMAIL, "ahlrichs@yahoo.com" };
	...
	public void testAll() throws Exception
	{
		...
		dirk.setPassword(RandomStringUtils.randomAlphabetic(8));
		...
		tom.setPassword(RandomStringUtils.randomAlphabetic(8));
	...
	public void testTaxCaching() throws JaloInvalidParameterException, ConsistencyCheckException, JaloPriceFactoryException
	{
		...
		try
		{
			...
			final EnumerationValue full = EnumerationManager.getInstance()
			                                                .createEnumerationValue(Europe1Constants.TC.PRODUCTTAXGROUP,
					                                                "test_full");
			final EnumerationValue half = EnumerationManager.getInstance()
			                                                .createEnumerationValue(Europe1Constants.TC.PRODUCTTAXGROUP,
					                                                "test_half");

			final EnumerationValue userGrp1 = EnumerationManager.getInstance()
			                                                    .createEnumerationValue(Europe1Constants.TC.USERTAXGROUP,
					                                                    "userGrp1");
			final EnumerationValue userGrp2 = EnumerationManager.getInstance()
			                                                    .createEnumerationValue(Europe1Constants.TC.USERTAXGROUP,
					                                                    "userGrp2");
			...
			assertThat(taxes).extracting(TaxValue::getValue).containsExactly(30d, 25d, 100d);
			assertThat(taxes).extracting(TaxValue::getCode).containsExactly(tax1.getCode(), tax1.getCode(), tax2.getCode());
			assertThat(taxes).extracting(TaxValue::isAbsolute).containsExactly(false, false, false);
			assertThat(taxes).extracting(TaxValue::getCurrencyIsoCode).containsExactly(null, null, null);
			...
			assertThat(taxes).extracting(TaxValue::getValue).containsExactly(100d, 40d);
			assertThat(taxes).extracting(TaxValue::getCode).containsExactly(tax2.getCode(), tax1.getCode());
			assertThat(taxes).extracting(TaxValue::isAbsolute).containsExactly(false, false);
			assertThat(taxes).extracting(TaxValue::getCurrencyIsoCode).containsExactly(null, null);
