...
public class UserSldTest extends ServicelayerBaseTest
{
	...
	public void shouldPasswordBeTheSameForItemAndModel() throws ConsistencyCheckException
	{
		...
		assertThat(passwordEncoderService.isValid(userModel, password)).isTrue();
		assertThat(userItem.checkPassword(password)).isTrue();
	...
	public void shouldGetDiscountRowsUsingSldAndRemoveOnlyOnesThatBelongToCustomer()
	{
		...
		final Customer cust1 = sessionService.executeInLocalView(new SessionExecutionBody()
		{
			@Override
			public Object execute()
			{
				JaloSession.getCurrentSession().getSessionContext().setLanguage(en);
