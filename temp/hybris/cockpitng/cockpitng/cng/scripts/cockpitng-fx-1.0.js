...
zk.afterLoad("zk", function () {
    ...
    zk.DnD.ghost = function (drag, offset, msg) {
        ...
        if (jq(drag.node).hasClass('yw-dashboard-slot-draggable')) {
            ...
            title.innerHTML = sanitizeHtml(drag.node.innerText);
...
var extendedSplitLayout = (function () {
    const class4BoxFirst = '.yw-extendedsplitlayout-box-first';
    const class4BoxThird = '.yw-extendedsplitlayout-box-third';
    const class4BoxWrapper = '.yw-extendedsplitlayout-box-wrapper';
    var widget = {};
    ...
    function makeResizable(uuid, layout) {
        ...
        if (layout === 'triple') {
            vesselChildren = vesselChildren.not(class4BoxThird);
        } else if (layout === 'twixe' || layout === 'twixs') {
            vesselChildren = vesselChildren.filter(class4BoxFirst);
        } else if (layout === 'mixleft') {
            vesselChildren = vesselChildren.filter(class4BoxWrapper);
            vesselChildren = vesselChildren.add(vesselChildren.children().filter(class4BoxFirst));
        } else if (layout === 'mixright') {
            const vesselWrapperFilter = vesselChildren.filter(class4BoxWrapper).children().filter('.yw-extendedsplitlayout-box-second');
            vesselChildren = vesselChildren.filter(class4BoxFirst);
    ...
    function findResizeReverse(elem, layout) {
        ...
        if (layout === 'mixleft') {
            if (elem.hasClass('yw-extendedsplitlayout-box-first')) {
                ...
                resizableSibling = elem.siblings(class4BoxThird);
            }
        } else if (layout === 'mixright') {
            if (elem.hasClass('yw-extendedsplitlayout-box-first')) {
                resizableSibling = elem.siblings(class4BoxWrapper);
...
const sapAttr4UiResizable = "ui-resizable";
const sapAttr4UiResizableReverse = "ui-resizable-alsoresize-reverse";

$.ui.plugin.add("resizable", "alsoResizeReverse", {
    start: function (event, ui) {

        const self = $(this).data(sapAttr4UiResizable), o = self.options,

            _store = function (exp) {
                $(exp).each(function () {
                    if (o.handles === "e") {
                        $(this).data(sapAttr4UiResizableReverse, {
    ...
    resize: function (event, ui) {
        const self = $(this).data(sapAttr4UiResizable), o = self.options, os = self.originalSize, op = self.originalPosition,
            delta = {
            ...
            _alsoResizeReverse = function (exp, c) {
                $(exp).each(function () {
                    var el = $(this),
                        start = $(this).data(sapAttr4UiResizableReverse),
                        style = {},
    ...
    stop: function (event, ui) {
        $(this).removeData(sapAttr4UiResizableReverse);
    }
});
