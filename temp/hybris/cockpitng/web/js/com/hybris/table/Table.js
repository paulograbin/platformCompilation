com.hybris.table.Table = zk.$extends(zk.Widget, {
	...
	requestStickyTableUpdate: function(loadingDelay){
	...
	setupRowContents: function(){
		...
		var _me = this;
		if(! $('#' + _me.parent.uuid).data('headerClasses')) {
			$('#' + _me.parent.uuid).data('headerClasses', true);
			window.setTimeout(function(){
				$('#' + _me.uuid + " ." + _me._classDef.headerCls + " ." + _me._classDef.headerCellCls + "_last").removeClass(_me._classDef.headerCellCls + "_last");
				$('#' + _me.uuid + " ." + _me._classDef.headerCls + " ." + _me._classDef.rowCls + ":first-child ." + _me._classDef.headerCellCls + ":not(." + _me._classDef.stickyHeaderCls + ")").last().addClass(_me._classDef.headerCellCls + "_last");
				$('#' + _me.parent.uuid).data('headerClasses', false);
			}, 440);
		}
    ...
    alignContentStates: function(id){
		if(this._alignStickyCheckbox){
			var _oc = $(id).find('.y-tablecell-header-sticky .z-checkbox');
			var _cc = $(id).next().find('.y-tablecell-header-sticky .z-checkbox');
			_oc.each(function(a){
				$(_cc[a]).children('input[type="checkbox"]').first().prop('checked', $(_oc[a]).children('input[type="checkbox"]').first().is(':checked'));			
			});
		}
    },
	...
	addScroll : function() {
		...
		const _scrollTarget = $(targetSelector).hasClass(_me.getZclass()) ? $(targetSelector) : false;
		const _scrolls = {
				scrollBarSize: false,
				...
				startScroll: function(cID,tID){
                    ...
                    if(!_scrollLay || _me._isContentChanged(tID)){
                    	...
                    	_me.alignContentStates(tID);
                    }
                    var _scr = this.isScrollable(cID, tID);
                    const _h = _scr.horizontal ? (_cont.height() - parseInt(this.getScrollBarWidth())) : _cont.height(),
                    	_w = _scr.vertical ? (_cont.width() - parseInt(this.getScrollBarWidth())) : _cont.width();
                    let	_t = _cont.offset().top,
                ...
                isScrollable: function(cID,tID) {
                    ...
                    this.updateScrollTarget(node, overflowX, scTarget);
                ...
                updateScrollTarget: function(node, overflowX, scTarget) {
                    if(scTarget){
                        if((overflowX === 'scroll' || overflowX === 'auto') && node.scrollWidth > (node.clientWidth - parseInt(this.getScrollBarWidth()))){
		...
		container[0].children[0].addEventListener('wheel', function(event) {
			var cID = "#" + this.parentElement.id,
				tID = "#" + this.id;
			event.preventDefault();
			event.stopPropagation();
			_scrolls.isScrollable(cID, tID);
			if(event.deltaY !== 0){
				this.parentElement.scrollTop += event.deltaY;
			}else{
				this.parentElement.scrollLeft += event.deltaX;
			}
